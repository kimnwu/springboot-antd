...
public class GenUtils {
    ...
    public static void genCode(ManageGenCode code) {
        ...
        String modelPath = JavaBeansUtil.getCamelCaseString(uri.replace(TAG_PATH, TAG.concat(TAG_PATH)), Boolean.FALSE).replace(TAG, TAG_PATH);// view页面引用model路径
        react.put("path", uri.replaceAll("[^/]*", "."));// sys/config生成如：../..
        react.put("modelPath", modelPath);
        react.put("namespace", JavaBeansUtil.getCamelCaseString(code.getUri(), Boolean.FALSE));// sys/config生成如：sysConfig
        data.put("_$", "$");
        data.put("_jj", "#");
        data.put("code", code);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                attrs.add(ManageGenAttr.builder()
                        ...
                        .jdbcName(jdbcName)
                        .javaName(JavaBeansUtil.getCamelCaseString(jdbcName, Boolean.FALSE))
                        .des(des == null ? "" : des)
                        .jdbcType(jdbcType)
