...
import com.ucsmy.commons.bean.LoginUser;
import com.ucsmy.commons.bean.MenuTree;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    public AuthenticationSuccessHandler loginSuccess() {
        return (request, response, authentication) -> {
            ...
            cacheService.delete(getLoginErrorKey(username));
    ...
    public AuthenticationFailureHandler loginFailure() {
        return (request, response, authentication) -> {
            ...
            // 登陆失败3次以上开启验证码
            if(StringUtils.isNotEmpty(username) && cacheService.incr(getLoginErrorKey(username), 30) > 2) {
                retMsg.setCode(1);
            }
            response.setContentType("application/json;charset=UTF-8");
    ...
    public AbstractUserDetailsAuthenticationProvider loginAuthenticationProvider() {
        return new AbstractUserDetailsAuthenticationProvider() {
            ...
            protected UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken authentication) throws AuthenticationException {
                ...
                // 图片验证码校验
                Integer errorNum = cacheService.get(getLoginErrorKey(username));
